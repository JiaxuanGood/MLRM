function [M_recover, M, N, iter] = PRLtrain(R_Omega, X, Y, lambda1, lambda2, mu1, mu2, maxiter, tol, LL)
...
if(sum(sum(Y))==length(Y))
    Y = eye(size(R_Omega,2));
else
    Y = (Y'*LL)';
end
